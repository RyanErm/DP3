...
python producer.py
```
The amount of time for this script is variable and can edited by altering the while loop.
...
```
Once it starts outputting that there are no messages left type "Command C" to stop the program. The program can also be stopped early with this command if desired. Then move onto the next script
...
```
Once it starts outputting that there are no messages left type "Command C" to stop the program. The program can also be stopped early with this command if desired. 
...
The updates API will give 1000-2000 data points per call. The positions API will give 300-500 data points per call. Allowing the producer script to run for 20-30 minutes should allow the program to collect at least 50,000 data points from both APIs. 
